...
const student* findstudent(const vector<student>& allstudents, int key) {
    ...
    while (getline(course_file, textline)) {
        if (state == 1) {
            ...
            try {
                ...
                cout << "Invalid course ID format" << endl;
                continue;
            }
            ...
            if (textline == "---------------------------------------") {
                ...
                int id = atoi(textline.c_str());
                if (id != 0) {
                    const student* p = findstudent(allstudents, id);
                    if (p != nullptr) {
                        allcourses.back().student_list.push_back(const_cast<student*>(p));
                    } else {
